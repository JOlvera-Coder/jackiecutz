...
# Mock database/state for Connected Business Bank
bank_account_info = {
    'bank_name': 'JPMorgan Chase Bank',
    'account_holder': 'Jackiecutz Salon Suite LLC',
    'routing_number': '*****0210',
    'account_number': '*************4489',
...
def stylist_dashboard():
    ...
    return render_template(
        ...
        bank=bank_account_info
    )

# --- BANKING & PAYOUT ROUTE ---
...
def save_bank_account():
    ...
    if bank_name and account:
        ...
        bank_account_info['account_holder'] = account_holder
        bank_account_info['routing_number'] = f"*****{routing[-4:]}" if len(routing) >= 4 else "*****"
        bank_account_info['account_number'] = f"*************{account[-4:]}" if len(account) >= 4 else "*****"
        bank_account_info['account_type'] = account_type
        bank_account_info['status'] = 'Verified & Connected 🟢'
    return redirect(url_for('main.stylist_dashboard'))

# --- SERVICE CATALOG CRUD ROUTES ---
...
# --- INVENTORY PRODUCT CATALOG CRUD ROUTES ---
...
def cashout_staff(staff_id):
    ...
    if s:
        ...
        if payout_amount > 0:
            exp = Expense(
                category="Payroll",
                description=f"Direct deposit payout for {s.name} via Connected Bank ({bank_account_info['bank_name']})",
...
print("Updated app/routes.py with Banking & Direct Deposit setup!")